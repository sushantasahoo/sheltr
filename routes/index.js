...
module.exports = function(app) {
	...
	app.post('/login', function(req, res) {
		...
		https.get({
			host : 'api.cloudmine.me',
			path : '/v1/app/60ecdcdd9fd6433297924f75c1c07b13/text?keys='
					+ req.body.user,
			headers : {
				'X-CloudMine-ApiKey' : process.env.CLOUDMINE
			}
	...
	app.get('/:idOrSlug', function(req, res, next) {
		...
		request('http://' + req.headers.host + '/api/loc/'
				+ req.params.idOrSlug, function(err, apiRes, body) {
			if (err)
				throw err;
		...
		// host: 'api.cloudmine.me',
		// path:
		// '/v1/app/60ecdcdd9fd6433297924f75c1c07b13/search?q=[slug="'+slug+'"]',
		// headers: {'X-CloudMine-ApiKey': process.env.CLOUDMINE}
		// }, function(cmres) {
		// var data = '';
		// cmres.on('data', function(chunk) {
